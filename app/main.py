...
async def api_error(_,exc:ApiError): return JSONResponse(exc.status,{'success':False,'error':{'code':exc.code,'message':exc.message,'details':exc.details}})
@app.exception_handler(RequestValidationError)
async def validation(_,exc): return JSONResponse(400,{'success':False,'error':{'code':'VALIDATION_ERROR','message':'Request validation failed','details':[{'field':'.'.join(str(x) for x in e['loc']),'reason':e['msg']} for e in exc.errors()]}})
...
async def http_error(_,exc):
    ...
    return JSONResponse(exc.status_code,{'success':False,'error':{'code':code,'message':str(exc.detail),'details':[]}})
@app.exception_handler(Exception)
async def unhandled(_,exc):
    log.exception('unhandled exception',exc_info=exc)
    return JSONResponse(500,{'success':False,'error':{'code':'INTERNAL_ERROR','message':'Internal server error','details':[]}})
